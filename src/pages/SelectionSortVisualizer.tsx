    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      for (let j = i + 1; j < n; j++) {
        ...
        if (arr[j].value < arr[minIndex].value) {
          if (minIndex !== i) {
            arr[minIndex].color = "hsl(var(--intermediate-primary))";
          }
          minIndex = j;
          arr[minIndex].color = "hsl(var(--intermediate-accent))";
          setArray([...arr]);
          await sleep(speed);
        } else {
      ...
      arr[i].color = "hsl(var(--intermediate-secondary))";
      setArray([...arr]);
      await sleep(speed);
    }

    arr[n - 1].color = "hsl(var(--intermediate-secondary))";
    setArray([...arr]);
    setIsSorting(false);
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: `hsl(var(--intermediate-bg))` }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-3 text-intermediate-primary">Selection Sort</h1>
          <p className="text-lg text-intermediate-text/70">
            Watch how the smallest elements are repeatedly selected and placed at the beginning.
          </p>
        </div>
