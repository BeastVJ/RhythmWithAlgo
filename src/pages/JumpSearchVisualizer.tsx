...
const JumpSearchVisualizer = () => {
  ...
  const handleSearch = async () => {
    ...
    // Jump phase
    while (curr < n && arr[curr - 1].value < target) {
      if (stopRef.current) break;

      arr.forEach((b, i) => (b.color = i >= prev && i < curr ? "yellow" : "hsl(var(--primary))"));
      setArray([...arr]);
      await sleep(speed);
    ...
    for (let i = prev; i < Math.min(curr, n); i++) {
      ...
      arr.forEach((b, idx) => (b.color = idx === i ? "orange" : "hsl(var(--primary))"));
      setArray([...arr]);
      await sleep(speed);
    ...
    <div className="min-h-screen bg-black text-white py-10 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-center mb-8 text-gray-400">
          Efficiently searches a **sorted array** by jumping fixed blocks and then doing linear search within the block.
        </p>
